...
export const getPortfolioData = cache(async () => {
  ...
  try {
    // Force dynamic fetching on every request by opting out of caching.
    // This ensures the data is always fresh from the remote URL at runtime.
    ...
    data = await res.json();
    console.log('Successfully fetched dynamic data from remote URL.');
  } catch (error) {
